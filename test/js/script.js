...
function errorMsg(text,end=true) {
...
async function clickConnect() {
    ...
    for (let tries = 1; tries < 5; tries++){
        ...
        try {
            if (await espTool.sync()) {
                ...
                // get our chip info 
                logMsg("Connected to " + await espTool.chipName() + " with attempt " + tries);
                if (debugState) {
                    console.log(espTool);
                }
    ...
    if(!isConnected){
        logMsg("Unable to connect after 5 tries...")
        await disconnect();
        toggleUIConnected(false);
        return;
    }
